...
function CreateSurfaceData(data)
{
    ...
    for (let i=0, ang = 0; i<iterCount; i++, ang+=step) {
        vertices.push( new Vertex( [r1 * Math.sin(deg2rad(ang)), zoffset+0, r1 * Math.cos(deg2rad(ang))], [0, 0] ));
        //vertices[vertices.length-1].normal = calculateNormal()
        vertices.push( new Vertex( [r2 * Math.sin(deg2rad(ang)), zoffset+0, r2 * Math.cos(deg2rad(ang))], [-ang/360, 0] ));
        //vertices[vertices.length-1].normal = calculateNormal()
        // FLOOR
    ...
    for (let i=0, ang = 0; i<iterCount; i++, ang+=step) {
        ...
        vertices.push( new Vertex( [r1 * Math.sin(deg2rad(ang)), zoffset+h, r1 * Math.cos(deg2rad(ang))], [0, h/hMax] ));
        //vertices[vertices.length-1].normal = calculateNormal()
        vertices.push( new Vertex( [r2 * Math.sin(deg2rad(ang)), zoffset+h, r2 * Math.cos(deg2rad(ang))], [-ang/360, h/hMax] )); 
        //vertices[vertices.length-1].normal = calculateNormal()
